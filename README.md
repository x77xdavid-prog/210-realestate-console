...
병원 건물 매입을 위한 매물 검토 콘솔입니다. 여러 공공 소스(온비드 공매·법원경매·LH 분양공고)에서 매물을 자동 수집하고, 조건에 맞는 신규 매물만 알림으로 보내며, 웹 대시보드에서 관심매물·매물장·체크리스트 검토·네이버 부동산 연동을 한 화면에서 관리합니다.

## 매물 수집 소스

| 소스 | 내용 | 방식 |
|------|------|------|
| **온비드 공매** | 양천·강서·구로·영등포구 압류·국유·수탁 재산 | data.go.kr 공식 API |
| **법원경매** | 서울남부지방법원 관할(양천·강서·구로·영등포) 부동산 경매 | courtauction.go.kr 검색(비공식, 사이트 개편 시 점검 필요) — 목록 수집 후 `selectAuctnCsSrchRslt`(사건 상세)·기일별 내역을 추가 조회, 경매 사진을 `<DB경로>/photos/` 에 압축 저장 |
| **LH 분양공고** | 서울 토지·상가 공급 공고 (신축 부지 후보) | data.go.kr 공식 API |
| **직접 등록** | 부동산에서 받은 매물 + 서류 첨부 | 대시보드 입력 |

네이버 부동산은 공식 API가 없어 자동 수집 대신 매물 위치를 새 탭으로 여는 링크 연동만 제공합니다.

### 경매 상세·사진·캘린더

법원경매 매물 카드를 클릭하면 `selectAuctnCsSrchRslt` API로 사건 상세(감정가·최저가·권리분석·기일별 내역)를 조회하는 상세 모달이 열립니다. 수집 시 경매 사진은 Pillow로 압축(최대 1280px, JPEG 품질 70)해 `<DB 경로>/photos/<사건번호>/` 에 저장되며, 대시보드는 `/api/photo?path=…` 엔드포인트로 서빙합니다 — 원본 사이트에 직접 핫링크하지 않습니다.

월별 경매일정 캘린더(`/api/calendar`)는 수집된 매물의 `매각기일(sale_date)` 필드를 연월별로 그룹화해 보여줍니다. 캘린더는 서버 추가 호출 없이 이미 로드된 매물 데이터에서 즉시 계산됩니다.

**추가된 API 엔드포인트**

| 경로 | 설명 |
|------|------|
| `/api/listing/detail` | 법원경매 사건 상세 조회 (`id`, `cs`, `court`, `seq` 파라미터) |
| `/api/photo` | 저장된 경매 사진 서빙 (`path` 파라미터) |
| `/api/calendar` | 월별 경매일정 데이터 (수집 매물 기준) |

**외부 의존성**: `Pillow` (경매 사진 압축 저장용). 설치되지 않은 경우 사진 저장을 건너뛰고 다른 기능은 정상 동작합니다.
...
- **체크리스트 검토**: 매물장 매물을 [병원건물 매입·신축 체크리스트](docs/병원건물_매입_신축_체크리스트.md) 기준으로 검토. 프로필 3종(기존 건물 사용/철거 후 신축/나대지 신축)별 항목이 다르며, 공공데이터로 자동 판정(용도지역·도로접면·주차·승강기·시세·노후도·**같은 동 정형외과 경쟁 의원·약국 현황** — 심평원 병원·약국정보서비스)하고 나머지는 수동 체크. 치명 항목 부적합 시 즉시 "부적합", 그 외 가중 점수로 A~D 등급(3D 플립 배지) 산출. 자동 검증 실행 시 6단계 진행 연출과 함께 추천 액션이 담긴 대시보드형 리포트 자동 생성
...
| 키 | 환경 변수 | 발급처 | 사용 데이터 |
|---|---|---|---|
| 공공데이터포털 | `DATA_GO_KR_API_KEY` | [data.go.kr](https://www.data.go.kr) 회원가입 → 아래 서비스에 각각 활용신청(자동승인) → 마이페이지 일반 인증키(Decoding) | [건축물대장(건축HUB)](https://www.data.go.kr/data/15134735/openapi.do), [상업업무용 실거래가](https://www.data.go.kr/data/15126463/openapi.do), [심평원 병원정보서비스](https://www.data.go.kr/data/15001698/openapi.do), [심평원 약국정보서비스](https://www.data.go.kr/data/15051059/openapi.do) |
...
키가 없으면 해당 소스만 건너뛰고 리포트에 발급 안내가 표시됩니다. 현재 지번 파싱은 양천구(목동·신정동·신월동)·관악구(봉천동·신림동·남현동) 기준이며, 다른 지역은 `realestate_alert/address.py`의 `BJDONG_CODES`에 법정동코드를 추가하면 됩니다. (심평원 의원·약국 조회는 법정동코드 없이 동 이름만으로 동작합니다.)
...
매물장의 `체크리스트` → `자동 검증 실행`을 누르면 주소 기반으로 공공데이터(건축물대장·토지이용·실거래·심평원 의원/약국)를 조회해 `자동 검증 근거` 카드를 채우고 점수·등급을 매깁니다.

API 키가 없거나 데이터가 없어 **미확인**으로 남은 카드는 `📝 미확인 직접 채우기` 버튼으로 여는 **수동 검증 입력 페이지**(`web/verify.html`)에서 부동산 제공 자료(투자제안서·브리핑) 근거로 직접 채울 수 있습니다.

- **물건 정보 입력 → 자동 작성**: 용도지역·준공연도·주차·승강기·연면적·주용도를 넣으면 용도지역·노후도·주차·승강기·현재용도 카드가 자동 판정됩니다.
- **카드별 상태·근거 직접 수정**: 시세·경쟁 의원·약국 등 외부 데이터가 필요한 카드는 직접 입력합니다.
- 수동 입력 값은 `제공 자료` 배지로 표시되고, **자동 검증을 다시 돌려도 보존**됩니다. 미확인으로 비워 저장하면 자동값으로 복귀합니다.
- **경쟁 의원·약국 자동조회**: 위 표의 심평원 두 서비스를 활용신청하고 `DATA_GO_KR_API_KEY`를 설정하면 같은 동 정형외과 의원 수·약국 수가 자동으로 채워집니다(법정동코드 미등록 지역도 동 이름으로 조회). 키 발급 전에는 페이지 안내에 따라 직접 입력하세요.
